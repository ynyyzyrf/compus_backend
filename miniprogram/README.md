...
小程序先用 `wx.login` 向後端確認微信身份。後端要求綁定手機號時，登入頁會
顯示真正的微信 `getPhoneNumber` 授權按鈕，並將獨立的手機號授權碼傳給後端。
登入頁也可直接點「使用手機號登入」驗證此流程，即使後端尚未強制綁定。
頁面不再顯示示例手機號。後端的 `verified_phone` 是登入身份，個人名片中的
「聯絡電話」仍可編輯，兩者分開存儲。正式啟用步驟見
[後端說明](../backend/README.md#已驗證手機號登入)。
